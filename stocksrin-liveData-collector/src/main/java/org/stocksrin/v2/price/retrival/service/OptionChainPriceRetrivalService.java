...
public class OptionChainPriceRetrivalService extends TimerTask {
	...
	public OptionChainPriceRetrivalService(boolean isForEveningDownLoad) {
	...
	public void update(String symbole) {
		...
		task(symbole, futurePrice);
	}

	public void task(String symbole, Map<String, Double> futurePrice) {
		...
		if (symbole.equalsIgnoreCase("NIFTY")) {
			...
			log.info(data.get(Data.shortedExpiry.get(0)).getSymbole() + ":"
	...
	public void filter(OptionModel optionModel) {
		...
		IntraDayOptionModel optionModelResult = new IntraDayOptionModel(optionModel.getSymbole());
	...
	public static void main(String[] args) {
		OptionChainPriceRetrivalService OptionChainPriceRetrivalService = new OptionChainPriceRetrivalService(false);
